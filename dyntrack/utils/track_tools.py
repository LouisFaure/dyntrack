...
def sinuosity_max_old(ldata, win=20):
    ...
    ldata["dist"] = np.sqrt(
        (ldata["Position X"] - dx) ** 2 + (ldata["Position Y"] - dy) ** 2
    )
    ldata["sinuosity"] = np.nan

    for w in range(0, (ldata.shape[0] - win)):
        ppd = np.sqrt(
            (ldata["Position X"].values[w + win] - ldata["Position X"].values[w]) ** 2
            + (ldata["Position Y"].values[w + win] - ldata["Position Y"].values[w]) ** 2
        )

        sinuosity = sum(ldata["dist"].values[(w + 1) : (w + win + 1)]) / ppd
        for i in range(w, w + win + 1):
            ldata["sinuosity"].values[i] = np.nanmax(
                [ldata["sinuosity"].values[i], sinuosity]
            )
...
def sinuosities_track(ldata):
    ...
    ppd = np.sqrt(
        (ldata["Position X"].values[len(dx) - 1] - ldata["Position X"].values[0]) ** 2
...
def tracksplot(files_c, files_v, ldata_e, sc, map_img, palette=None):
    if len(files_c) > 1:
        rown = round((np.sqrt(len(files_c))) / 2.0) * 2
    ...
    for i in range(0, len(files_c)):
        ...
        a = (
            np.linspace(
                min(pd.concat(ldata_e)["Position X"]),
                max(pd.concat(ldata_e)["Position X"]),
                gs,
            )
            / sc
            / 355
        ...
        b = (
            np.linspace(
                min(pd.concat(ldata_e)["Position Y"]),
                max(pd.concat(ldata_e)["Position Y"]),
                gs,
            )
            / sc
            / 355
        ...
        if len(files_c) > 1:
            for cu in curves[0]:
                plt.plot(
                    ldata_e[cu]["Position X"] / sc / 355,
                    ldata_e[cu][["Position Y"]] / sc / 355,
                    c=palette[i],
                    linewidth=0.5,
                )
            plt.gca().invert_yaxis()
            plt.quiver(out[0], out[1], vect[0], -vect[1], alpha=0.7)
        else:
...
def distances_accelerations_old(ldat):
    dst = list(
        map(
            lambda x: np.sqrt(
                (ldat.values[x, 0] - ldat.values[x + 1, 0]) ** 2
                + (ldat.values[x, 1] - ldat.values[x + 1, 1]) ** 2
            ),
